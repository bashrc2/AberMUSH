...
def describeTrappedPlayer(mud,id,players,rooms):
    ...
    if rooms[roomID]['trap']['trapType']=='chain net':
        mud.send_message(id, randomDescription("You struggle with the net but its chain webbing is pinning you down|You seem to be trapped in a net made from linked chains|Being covered by a chain net makes it difficult to move")+'.\n\n')
...
def escapeWithCuttingTool(mud,id,players,rooms,itemsDB):
    ...
    if itemID==-1:
        mud.send_message(
            id, randomDescription(
                "You attempt to escape with your bare hands, but remain trapped|You tug and wrestle but can't escape|Looks like you need to use a cutting tool, otherwise you'll be here for a while|Looks like you need something to cut with") + '.\n\n')
        return
    roomID=players[id]['room']
    ...
    if trapDamage < maxTrapDamage:
        ...
        mud.send_message(
            id, randomDescription('You slash at the ' + rooms[roomID]['trap']['trapType'] + damageStr + '|You cut the ' + rooms[roomID]['trap']['trapType'] + damageStr) + '.\n\n')
    else:
        rooms[roomID]['trap']['trapDamaged']=0
        rooms[roomID]['trap']['trappedPlayers'].clear()
        if len(rooms[roomID]['trap']['trapEscapeDescription'])>0:
            mud.send_message(
                id, randomDescription(rooms[roomID]['trap']['trapEscapeDescription']) + '.\n\n')
        else:
            mud.send_message(
                id, randomDescription('You cut a large hole in the ' + rooms[roomID]['trap']['trapType'] + ' and escape') + '.\n\n')
...
def runTraps(mud,rooms,players,npcs):
    ...
    for rm in rooms:
        ...
        if rooms[rm]['trap']['trapActivationTime']==0:
            continue
        now = int(time.time())
        timeElapsed=now - rooms[rm]['trap']['trapActivationTime']
        if now >= rooms[rm]['trap']['trapActivationTime'] + TimeStringToSec(rooms[rm]['trap']['trapDuration']):
