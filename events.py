...
def spawnNPC(etarget, ebody, players, npcs, items, env, npcsDB, envDB):
        ...
        # print(npcsDB)
        # print(body)
        npcs[getFreeKey(npcs, 90000)] = { 'name': npcsDB[int(body[0])]['name'], 'room': body[1], 'lvl': npcsDB[int(body[0])]['lvl'], 'exp': npcsDB[int(body[0])]['exp'], 'str': npcsDB[int(body[0])]['str'], 'siz': npcsDB[int(body[0])]['siz'], 'wei': npcsDB[int(body[0])]['wei'], 'per': npcsDB[int(body[0])]['per'], 'endu': npcsDB[int(body[0])]['endu'], 'cha': npcsDB[int(body[0])]['cha'], 'inte': npcsDB[int(body[0])]['inte'], 'agi': npcsDB[int(body[0])]['agi'], 'follow': npcsDB[int(body[0])]['follow'], 'luc': npcsDB[int(body[0])]['luc'], 'cred': npcsDB[int(body[0])]['cred'], 'inv': npcsDB[int(body[0])]['inv'], 'path': npcsDB[int(body[0])]['path'], 'moveDelay': npcsDB[int(body[0])]['moveDelay'], 'moveType': npcsDB[int(body[0])]['moveType'], 'isAttackable': int(body[2]), 'isStealable': int(body[3]), 'isKillable': int(body[4]), 'isAggressive': int(body[5]), 'clo_head': npcsDB[int(body[0])]['clo_head'], 'clo_larm': npcsDB[int(body[0])]['clo_larm'], 'clo_rarm': npcsDB[int(body[0])]['clo_rarm'], 'clo_lhand': npcsDB[int(body[0])]['clo_lhand'], 'clo_rhand': npcsDB[int(body[0])]['clo_rhand'], 'clo_chest': npcsDB[int(body[0])]['clo_chest'], 'clo_lleg': npcsDB[int(body[0])]['clo_lleg'], 'clo_rleg': npcsDB[int(body[0])]['clo_rleg'], 'clo_feet': npcsDB[int(body[0])]['clo_feet'], 'imp_head': npcsDB[int(body[0])]['imp_head'], 'imp_larm': npcsDB[int(body[0])]['imp_larm'], 'imp_rarm': npcsDB[int(body[0])]['imp_rarm'], 'imp_lhand': npcsDB[int(body[0])]['imp_lhand'], 'imp_rhand': npcsDB[int(body[0])]['imp_rhand'], 'imp_chest': npcsDB[int(body[0])]['imp_chest'], 'imp_lleg': npcsDB[int(body[0])]['imp_lleg'], 'imp_rleg': npcsDB[int(body[0])]['imp_rleg'], 'imp_feet': npcsDB[int(body[0])]['imp_feet'], 'hp': npcsDB[int(body[0])]['hp'], 'charge': npcsDB[int(body[0])]['charge'], 'corpseTTL': int(body[6]), 'respawn': int(body[7]), 'vocabulary': npcsDB[int(body[0])]['vocabulary'], 'talkDelay': npcsDB[int(body[0])]['talkDelay'], 'inDescription': npcsDB[int(body[0])]['inDescription'], 'outDescription': npcsDB[int(body[0])]['outDescription'], 'lookDescription': npcsDB[int(body[0])]['lookDescription'], 'timeTalked': 0, 'isInCombat': 0, 'lastCombatAction': 0, 'lastRoom': None, 'whenDied': None, 'randomizer': 0, 'randomFactor': npcsDB[int(body[0])]['randomFactor'], 'lastSaid': 0, 'lastMoved': 0 }
